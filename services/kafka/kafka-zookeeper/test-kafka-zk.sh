...
docker_kafka_command() {
    ...
    if check_docker; then
        ...
        if [ -n "$kafka_container" ]; then
            ...
            docker exec $kafka_container bash -c "$cmd $args" 2>/dev/null || \
            docker exec $kafka_container bash -c "/usr/bin/$cmd $args"
            return $?
        else
            print_warning "未找到运行中的Kafka容器，使用临时容器并连接到kafka-network网络"
            # 检查是否存在kafka-network
            if docker network ls | grep -q kafka-network; then
                docker run --rm --network kafka-network confluentinc/cp-kafka:7.5.0 bash -c "$cmd $args" 2>/dev/null || \
                docker run --rm --network kafka-network confluentinc/cp-kafka:7.5.0 bash -c "/usr/bin/$cmd $args"
            else
                docker run --rm confluentinc/cp-kafka:7.5.0 bash -c "$cmd $args" 2>/dev/null || \
                docker run --rm confluentinc/cp-kafka:7.5.0 bash -c "/usr/bin/$cmd $args"
            fi
            return $?
        fi
    else
